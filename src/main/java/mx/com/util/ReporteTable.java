 ...
 * @author abigail
 */
//clase para almacenar los datos de la tabla de reporte de trabajadores
public class ReporteTable extends AbstractTableModel {
    ...
    private List<Map<String, String>> usuario = null; //en esta lista guardamos todos los datos que contiene la tabla
    
    //constructor de la clase que recibe una lista con todos los datos de la tabla
    public ReporteTable(List<Map<String, String>> usuario) {
        this.usuario= usuario; //se asigna esa lista en la lista de esta clase
        if(usuario.size()>0){ //si es mayor que 0 significa que si tenemos datos
            //obtenemos la primera fila de la tabla (los titulos o columnas)
            columnas = usuario.get(0).keySet().toArray(new String[usuario.get(0).size()]); 
    ...
    public Object getValueAt(int row, int col) {
        if (dataLoaded){ //si hay valores en la tabla
            return usuario.get(row).get(columnas[col]); //se obtienen todas las filas y columnas
        }
